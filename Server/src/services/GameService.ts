...
import { PokemonInstanceModel, IPokemonInstance } from '../models/PokemonInstance';
import { PokemonStatsModel } from '../models/PokemonStats';
...
export class GameService {
   ...
   */
  public static async healPlayerParty(playerId: string | Types.ObjectId): Promise<boolean> {
    try {
      // Find all pokemon in the player's party
      const party = await PokemonInstanceModel.find({ 
        owner_id: playerId, 
        is_in_party: true 
      });

      if (party.length === 0) return true; // Nothing to heal
      ...
      await PokemonInstanceModel.bulkWrite(bulkOps);
      return true;
    } catch (error) {
  ...
  public static async catchPokemon(
    ...
    try {
      ...
      const partyCount = await PokemonInstanceModel.countDocuments({ owner_id: playerId, is_in_party: true }).session(session);
      const is_in_party = partyCount < 6;
      const party_slot = is_in_party ? partyCount + 1 : undefined;

      // Step 3: Insert into PokemonInstance
      const newPokemon = new PokemonInstanceModel({
        owner_id: playerId,
      ...
      const newStats = new PokemonStatsModel({
  ...
  public static async executeTrade(
    player1Id: string | Types.ObjectId,
    player2Id: string | Types.ObjectId,
    pokemonInstanceId1: string | Types.ObjectId,
    pokemonInstanceId2: string | Types.ObjectId
  ): Promise<boolean> {
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const p1Pokemon = await PokemonInstanceModel.findOne({ 
        _id: pokemonInstanceId1, 
        owner_id: player1Id 
      }).session(session);

      const p2Pokemon = await PokemonInstanceModel.findOne({ 
        _id: pokemonInstanceId2, 
        owner_id: player2Id 
      }).session(session);

      if (!p1Pokemon) throw new Error('Player 1 does not own the specified Pokemon.');
      if (!p2Pokemon) throw new Error('Player 2 does not own the specified Pokemon.');

      // Step 2: Swap owners & remove from party (to prevent invalid party slots)
      p1Pokemon.owner_id = new mongoose.Types.ObjectId(player2Id.toString());
      p1Pokemon.is_in_party = false;
      p1Pokemon.party_slot = undefined;

      p2Pokemon.owner_id = new mongoose.Types.ObjectId(player1Id.toString());
   ...
   */
  public static async checkCanEnterZone(
    playerId: string | Types.ObjectId, 
    ...
    try {
      ...
      const player = await PlayerModel.exists({
        _id: playerId,
   ...
   */
  public static async onBossDefeated(
    playerId: string | Types.ObjectId, 
    bossId: string
  ): Promise<void> {
    try {
